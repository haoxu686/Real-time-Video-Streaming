...
public class ImageExplorerActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		repository = GlobalDef.conf.HOME+GlobalDef.conf.IMAGE_PATH;
	...
	private class GalleryAdapter extends BaseAdapter {
		...
		public GalleryAdapter(Activity activity, File [] fileList) {
			...
			for (int i = 0; i < fileList.length; i++) {
				if (fileList[i].isDirectory() || !FileUtil.getMIMEType(fileList[i].getName()).equals("image")) {
					continue;
				}
		...
		public View getView(int arg0, View arg1, ViewGroup arg2) {
			ImageView view = null;
			Bitmap image = BitmapFactory.decodeFile(files.get(arg0).getAbsolutePath(), null);
			if (arg1 != null) {
				view = (ImageView) arg1;
